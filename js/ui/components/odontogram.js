...
export class Odontogram {
    ...
    render() {
        ...
        this.container.innerHTML = `
            ...
            <div id="odont-face-modal" class="fixed inset-0 bg-dark/60 backdrop-blur-md z-[1000] flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300 p-4">
                <div class="bg-white rounded-[28px] shadow-2xl scale-95 transition-all duration-300 w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col" id="odont-face-container">
    ...
    openClinicalModal(id) {
        ...
        container.innerHTML = `
            <div class="p-8 border-b border-slate-100 flex justify-between items-start bg-slate-50">
                <div>
                    <h3 class="text-2xl font-display font-extrabold text-dark tracking-tight">Diente ${id}</h3>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Plan de Tratamiento (${categoryFilter})</p>
                </div>
                <button onclick="window._odontInstance.closeClinicalModal()" class="w-10 h-10 bg-white shadow-soft rounded-full flex items-center justify-center text-slate-400 hover:text-dark">
            ...
            <div class="p-8 space-y-8 flex-grow overflow-y-auto custom-scrollbar">
                ...
                <div class="flex gap-4">
                    <button id="st-caries" onclick="window._odontInstance.setStatus('caries')" class="status-btn flex-1 p-4 rounded-xl border-2 border-red-100 bg-red-50 text-red-600 font-bold text-xs uppercase tracking-widest flex flex-col items-center gap-2 active ring-4 ring-red-50">
                        <span class="w-3 h-3 rounded-full bg-red-500"></span> Por Realizar
                    </button>
                    <button id="st-realizado" onclick="window._odontInstance.setStatus('realizado')" class="status-btn flex-1 p-4 rounded-xl border-2 border-slate-100 bg-slate-50 text-slate-400 font-bold text-xs uppercase tracking-widest flex flex-col items-center gap-2">
                        <span class="w-3 h-3 rounded-full bg-blue-500"></span> Existente
                    </button>
                </div>
                ...
                <div class="space-y-4">
                    <h4 class="text-[11px] font-bold text-primary uppercase tracking-widest flex items-center gap-2">
                        <span class="w-1 h-3 bg-primary rounded-full"></span> Grupo A: Restauración x Caras
                    </h4>
                    <div id="faces-selector" class="grid grid-cols-5 gap-2">
                        ${['V', 'L', 'M', 'D', 'O'].map(f => `
                            <button onclick="window._odontInstance.toggleFace('${f}')" id="face-${f}" class="p-3 border rounded-lg text-[10px] font-bold text-slate-500 hover:border-primary transition-all">
                ...
                <div class="space-y-4">
                    <h4 class="text-[11px] font-bold text-primary uppercase tracking-widest flex items-center gap-2">
                        <span class="w-1 h-3 bg-accent rounded-full"></span> Grupo B y C: Procedimientos Clínicos
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3" id="procedimientos-list">
                        <!-- Loaded from catalog -->
                    </div>
                </div>
            </div>
            ...
            <div class="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                <button onclick="window._odontInstance.clearTooth('${id}')" class="sach-button variant-unset !h-12 !px-6 border-red-200 text-red-400">Limpiar Diente</button>
                <button onclick="window._odontInstance.applyClinicalChanges()" class="sach-button variant-set bg-primary !h-12 w-full shadow-glow">Finalizar Procedimiento</button>
            </div>
        `;
