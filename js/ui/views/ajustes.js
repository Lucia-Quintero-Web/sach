...
export function openAjustesModal() {
    ...
    overlay.innerHTML = `
        ...
        <div class="relative bg-white rounded-[32px] shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-in zoom-in-95 fade-in duration-400 overflow-hidden">
            ...
            <!-- Search bar -->
            <div class="px-8 py-4 border-b border-black/5 flex-shrink-0 bg-slate-50/60">
                <div class="relative">
                    <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-width="2"/></svg>
                    <input id="ajustes-search" type="text" placeholder="Filtrar tratamiento..." oninput="window.filterTratamientos(this.value)"
                           class="w-full pl-11 pr-4 py-3 text-sm font-medium bg-white border border-black/5 rounded-xl outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all text-dark placeholder:text-slate-300">
                </div>
            </div>
