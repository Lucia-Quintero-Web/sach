    ...
    content.innerHTML = `
        ...
        <div class="flex items-center justify-between border-b border-black/5 mb-8">
            ...
            <div class="flex items-center gap-2">
                <button onclick="window.exportPdfPresupuesto('${p.id}')" class="flex items-center shadow-glow gap-1.5 px-3 py-2 bg-accent text-white rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-[#00a892] transition-all">
...
window.printClinicalRecord = async (pId) => {
    ...
    try {
        const tmpOdont = new Odontogram(hiddenContainer.id, p.odontograma_json, null, p);
        tmpOdont.render();
    ...
    printWin.document.write(`
        ...
        <html>
            <head>
                <title>REPORTE CLÍNICO - ${p['NOMBRE DEL PACIENTE']}</title>
                ...
                <div class="page">
                    ...
                    <div class="patient-card">
                        <div class="info-group">
                            ...
                            <p class="value">${p['NOMBRE DEL PACIENTE']}</p>
                        </div>
                        <div class="info-group">
                            <p class="label">Cédula / ID</p>
                            <p class="value">${p['ID DEL PACIENTE']}</p>
                        </div>
                        <div class="info-group">
                        ...
                        <div class="info-group">
                            ...
                            <p class="value">${p['EDAD']} años</p>
                        </div>
                        <div class="info-group">
                            <p class="label">Género</p>
                            <p class="value">${p['SEXO']}</p>
                        </div>
                        <div class="info-group">
...
window.exportPdfPresupuesto = async (pId) => {
    if (!window.html2pdf) {
        alert("Librería de PDF no cargada. Actualice la página e intente de nuevo.");
        return;
    }
    ...
    // Overlay de Carga (UX Neo-Medical)
    const loadingOverlay = document.createElement('div');
    loadingOverlay.id = 'pdf-loading-overlay';
    loadingOverlay.className = 'fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex flex-col items-center justify-center text-white transition-opacity duration-300';
    loadingOverlay.innerHTML = `
        <svg class="animate-spin h-10 w-10 text-accent mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        <h3 class="text-xl font-bold tracking-wide">Preparando PDF Institucional</h3>
        <p class="text-sm text-slate-300 mt-2 font-medium">Renderizando gráficos clínicos...</p>
    `;
    document.body.appendChild(loadingOverlay);

    // Yield control to browser unblock UI thread
    setTimeout(() => {
        let b = p.presupuesto_json || { items: [], abonos: [], total_abonado: 0 };
        if (Array.isArray(b)) b = { items: b, abonos: [], total_abonado: 0 };
        const items = b.items || [];
        ...
        // Render Odontograma Hidden for Snapshot
        const hiddenContainer = document.createElement('div');
        hiddenContainer.id = `odont-pdf-${pId}`;
        hiddenContainer.style.width = '800px';
        hiddenContainer.style.position = 'absolute';
        hiddenContainer.style.left = '-9999px';
        hiddenContainer.style.top = '0';
        ...
        try {
            ...
            if (odontNode) {
                odontNode.style.boxShadow = 'none';
                odontNode.style.border = '1px solid #e2e8f0';
                const m = hiddenContainer.querySelector('#odont-face-modal');
                if (m) m.remove();
                odontHtml = odontNode.outerHTML;
            }
        } catch (e) {
            console.warn('Fallback a estado vacío o captura actual.', e);
            const existingOdont = document.querySelector('.odontogram-wrapper');
            if (existingOdont) {
                const clone = existingOdont.cloneNode(true);
                clone.style.boxShadow = 'none';
                clone.style.border = '1px solid #e2e8f0';
                odontHtml = clone.outerHTML;
            }
        }

        const content = `
            <div style="padding: 40px; font-family: 'Inter', sans-serif; color: #0f172a; width: 800px; background: white; margin: 0 auto; min-height: 1000px;">
                <!-- Encabezado -->
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #00BFA6; padding-bottom: 20px; margin-bottom: 30px;">
                <div style="background-color: #0F4C5C; width: 60px; height: 60px; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: white; font-weight: 900; font-size: 24px; font-family: 'Inter', sans-serif;">
                    LQ<span style="color: #00BFA6;">.</span>
                </div>
                <div style="text-align: right;">
                    <h2 style="margin: 0; font-size: 20px; color: #0F4C5C; font-weight: 800;">Plan de Tratamiento y Presupuesto</h2>
                    <p style="margin: 4px 0 0 0; color: #64748b; font-size: 11px; font-weight: bold; text-transform: uppercase;">Emisión: ${new Date().toLocaleDateString()}</p>
                </div>
            </div>

                <!-- Datos Paciente -->
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px; background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #f1f5f9;">
                    <div>
                        <div style="font-size: 9px; color: #94a3b8; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em;">Paciente</div>
                        <div style="font-size: 14px; font-weight: 700; color: #1e293b;">${p['NOMBRE DEL PACIENTE']}</div>
                    </div>
                    <div>
                        <div style="font-size: 9px; color: #94a3b8; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em;">Cédula / Identificación</div>
                        <div style="font-size: 14px; font-weight: 700; color: #1e293b;">${p['ID DEL PACIENTE']}</div>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 30px; page-break-inside: avoid;">
                    <div style="font-size: 11px; color: #0F4C5C; font-weight: 800; text-transform: uppercase; margin-bottom: 12px; border-left: 3px solid #00BFA6; padding-left: 8px;">Estado Clínico Gráfico</div>
                ...
                <div style="margin-bottom: 40px; page-break-inside: avoid;">
                    <div style="font-size: 11px; color: #0F4C5C; font-weight: 800; text-transform: uppercase; margin-bottom: 12px; border-left: 3px solid #00BFA6; padding-left: 8px;">Detalle de Inversión</div>
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: #0F4C5C; color: white;">
                                <th style="text-align: left; padding: 10px 12px; font-size: 10px; text-transform: uppercase;">Tratamiento / Procedimiento</th>
                                <th style="text-align: right; padding: 10px 12px; font-size: 10px; text-transform: uppercase;">Valor (USD)</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${items.length === 0 ? '<tr><td colspan="2" style="padding: 15px; text-align: center; color: #94a3b8; font-size: 12px;">Ningún tratamiento ingresado.</td></tr>' : ''}
                            ${items.map(i => `
                                <tr>
                                    <td style="padding: 10px 12px; border-bottom: 1px solid #f1f5f9; font-size: 12px; font-weight: 600; color: #334155;">
                                        ${i.nombre} 
                                        ${i.diente ? `<span style="color: #64748b; font-size: 10px; margin-left: 6px; font-weight: 500;">(Diente: ${i.diente})</span>` : ''}
                                    </td>
                                    <td style="text-align: right; padding: 10px 12px; border-bottom: 1px solid #f1f5f9; font-size: 12px; font-weight: 700; color: #1e293b;">
                                        $${parseFloat(i.costo).toFixed(2)}
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
                ...
                <div style="display: flex; justify-content: flex-end; margin-bottom: 40px; page-break-inside: avoid;">
                    <div style="width: 280px; background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #f1f5f9;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 6px; font-size: 13px; font-weight: 700; color: #475569;">
                            <span>Inversión Total:</span>
                            <span>$${total.toFixed(2)}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 6px; font-size: 13px; color: #059669; font-weight: 700;">
                            <span>Abonos Realizados:</span>
                            <span>-$${totalAbonado.toFixed(2)}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; margin-top: 10px; padding-top: 10px; border-top: 2px solid #e2e8f0; font-size: 16px; font-weight: 800; color: #0F4C5C;">
                ...
                <div style="margin-top: 80px; text-align: center; page-break-inside: avoid;">
                    <div style="width: 250px; border-top: 2px solid #cbd5e1; margin: 0 auto; padding-top: 10px;">
                        <p style="margin: 0; font-weight: 800; font-size: 14px; color: #0F4C5C;">Dra. Lucía Quintero</p>
                        <p style="margin: 0; font-size: 10px; color: #64748b; font-weight: 600; text-transform: uppercase;">Especialista Odontopediatría</p>
                    </div>
                    <p style="margin-top: 40px; font-size: 9px; color: #94a3b8; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;">Este presupuesto posee una validez oficial de 15 días tras su fecha de emisión.</p>
                </div>
            </div>
        `;
        ...
        const opt = {
            margin: 0,
            filename: `Presupuesto_SACH_${p['NOMBRE DEL PACIENTE'].replace(/\s+/g, '_')}.pdf`,
        ...
        finalContainer.style.left = '-9999px';
        finalContainer.style.top = '0';
        finalContainer.style.width = '800px';
        finalContainer.style.background = 'white';
        finalContainer.innerHTML = content;
        document.body.appendChild(finalContainer);

        // EXTRA CRITICAL: Wait for browser to actually paint the hidden content
        // html2pdf will capture empty if we don't wait for layout engine
        setTimeout(() => {
            const opt = {
                margin: 0,
                filename: `Presupuesto_SACH_${p['NOMBRE DEL PACIENTE'].replace(/\s+/g, '_')}.pdf`,
            ...
            html2pdf().set(opt).from(finalContainer).save().then(() => {
                ...
                if (p['CELULAR DEL PACIENTE']) {
                    setTimeout(() => {
                        const msgWA = `Hola ${p['NOMBRE DEL PACIENTE']}. Soy la Dra. Lucía Quintero, te adjunto el *Plan de Tratamiento y Presupuesto Odontológico Neo-Medical* en PDF 📄🦷. \n\nPuedes enviarme el documento descargado aquí mismo para validarlo.\n\nQuedo atenta, ¡que tengas un gran día!✨`;
                        const waUrl = `https://api.whatsapp.com/send?phone=${sanitizarCelularEcuador(p['CELULAR DEL PACIENTE'])}&text=${encodeURIComponent(msgWA)}`;

                        if (confirm('✅ PDF Generado y Descargado Exitosamente.\n\n¿Desea enviar una notificación por WhatsApp al paciente para que le envíe este PDF?')) {
                    ...
                    setTimeout(() => {
                        alert('✅ PDF Generado Exitosamente.\n\n⚠️ El paciente no tiene un celular registrado. No se puede enviar WhatsApp automático.');
                    }, 500);
                }
            }).catch(err => {
        ...
        }, 500); // 500ms delay to force DOM layout/painting before html2pdf starts capture
    }, 150); // Small 150ms delay to allow DOM to paint the Loading Overlay
};
