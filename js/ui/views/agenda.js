...
// Generar slots de 30 mins (08:00 a 20:00) = 24 slots
const SLOTS = [];
for (let h = 8; h < 20; h++) {
            ...
            <div class="sach-modal w-full max-w-lg transform scale-95 transition-all duration-300" id="apt-modal-container">
                ...
                <form id="new-appointment-form" class="space-y-6">
                    ...
                    <div class="sach-input-container">
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2.5 ml-2">Paciente Seleccionado</p>
                        <div id="apt-patient-display" class="sach-input py-3.5 h-12 font-bold text-dark bg-slate-50 border-none flex items-center px-4 rounded-xl">
...
async function setupAgendaLogic() {
    ...
    if (preselectedId) {
        ...
        if (p) {
            ...
            document.getElementById('apt-patient-display').innerText = `${p['NOMBRE DEL PACIENTE']} (CC: ${p['ID DEL PACIENTE']})`;
            document.getElementById('apt-patient-display').classList.replace('bg-slate-50', 'bg-primary/5');
            document.getElementById('apt-patient-display').classList.add('text-primary');
...
window.closeAptModal = () => {
    ...
    document.getElementById('apt-patient-display').innerText = 'Seleccione un paciente...';
    document.getElementById('apt-patient-display').classList.remove('text-primary', 'bg-primary/5');
    document.getElementById('apt-patient-display').classList.add('bg-slate-50');
};
            ...
            <!-- Acción -->\n            <div class="flex-shrink-0">
                ...
                : waUrl
                    ? `<a href="${waUrl}" target="_blank"
                               ...
                               class="flex items-center gap-1.5 px-3 py-2 bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366] hover:text-white rounded-[10px] transition-all text-[10px] font-bold uppercase tracking-widest">
